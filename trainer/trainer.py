...
class BLIPTrainer:
    ...
    def __init__(self, cfg, generator) -> None:
        self.cfg = cfg
        self.model_name = self.cfg.generate_model
        self.generator = generator
        self.df = load_data('./dataset_class/final_final_prompt.csv')
        if self.cfg.gradient_checkpoint:
            self.save_parameter = f'(best_score){str(self.cfg.generate_model)}_state_dict.pth'
    ...
    def train_fn(self, loader_train, model, criterion, optimizer, lr_scheduler):
        ...
        losses = AverageMeter()
        model.train()
        for step, prompt in enumerate(tqdm(loader_train)):
            optimizer.zero_grad()
    ...
    def valid_fn(self, loader_valid, model, val_metrics) -> float:
        ...
        # style_model.eval()
        with torch.no_grad():
            for step, prompt in enumerate(tqdm(loader_valid)):
                for k, v in prompt.items():
                    prompt[k] = v.to(self.cfg.device)
                pred_prompt = model(prompt)
                # val_metric = val_metrics(image_features[i], text_features[i]).mean()
                # metrics.update(val_metric.detach(), 1)
